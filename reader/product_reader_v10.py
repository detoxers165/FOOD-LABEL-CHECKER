...
def display_merged(m):
    ...
    for name, d in m["nutrition"].items():
        print(failures else 0


if __name__ == "__main__":
    sys.exit(main())
f"{name:22} : {d.get('values', d.get('value'))}")
...
def main():
    ...
    ap.add_argument("--merge", action="store_true",
                    help="treat ALL input images as one product and combine "
                         "them into a single record (front + back + nutrition panel)")
    ...
    for path in paths:
        ...
        try:
            ...
            if not args.quiet and not args.merge:
